...
<div id="comp-content" style="display:none"></div>`;
  ...
  // Works on any page — uses app.js modal if available, else creates its own
  const _titleEl  = document.getElementById('modal-title');
  const _bodyEl   = document.getElementById('modal-body-inner');
  const _footerEl = document.getElementById('modal-footer');
  const _overlayEl= document.getElementById('modal-overlay');
  const _footerHtml = `<button onclick="_compModalClose()" style="background:none;border:1px solid var(--border,#E8ECF0);color:var(--text2,#2C3E50);padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:600">Close</button>
    <button onclick="exportCompletenessCSV()" style="background:var(--blue,#4A90D9);color:#fff;border:none;padding:8px 18px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:600"><i class="fa-solid fa-download"></i> Export CSV</button>`;
  if (_titleEl && _bodyEl && _footerEl && _overlayEl) {
    _titleEl.textContent = 'Data Completeness Report';
    _bodyEl.innerHTML = body;
    _footerEl.innerHTML = _footerHtml;
    _overlayEl.classList.add('open');
  } else {
    document.getElementById('_comp-modal-overlay')?.remove();
    const _ov = document.createElement('div');
    _ov.id = '_comp-modal-overlay';
    _ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:5000;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(3px)';
    _ov.innerHTML = `<div style="background:var(--card,#fff);border-radius:14px;width:680px;max-width:95vw;max-height:90vh;display:flex;flex-direction:column;box-shadow:0 24px 80px rgba(0,0,0,.25);border:1px solid var(--border,#E8ECF0)">
      <div style="padding:16px 20px;border-bottom:1px solid var(--border,#E8ECF0);display:flex;align-items:center;justify-content:space-between;flex-shrink:0">
        <div style="font-size:15px;font-weight:700;color:var(--text,#0B1829)">Data Completeness Report</div>
        <button onclick="_compModalClose()" style="background:none;border:none;color:var(--text3,#5A6A7A);font-size:18px;cursor:pointer;width:28px;height:28px;border-radius:6px;display:flex;align-items:center;justify-content:center">✕</button>
      </div>
      <div id="comp-loading" style="padding:20px;overflow-y:auto;flex:1">${body}</div>
      <div style="padding:12px 20px;border-top:1px solid var(--border,#E8ECF0);display:flex;justify-content:flex-end;gap:8px;flex-shrink:0">${_footerHtml}</div>
    </div>`;
    _ov.addEventListener('click', e => { if (e.target === _ov) _compModalClose(); });
    document.body.appendChild(_ov);
  }
  window._compModalClose = function() {
